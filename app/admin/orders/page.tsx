...
export default function AdminOrdersPage() {
  ...
  const [showOrderActionsMenuId, setShowOrderActionsMenuId] = useState<
    string | null
  >(null);
  // Edit order handler
    ...
    <div className="space-y-6 bg-black min-h-screen p-6">
      ...
      {selectedOrderIds.length > 0 && (
        <div className="fixed top-0 left-0 w-full z-40 bg-[#18181b] border-b-2 border-[#f8c017] shadow-lg flex items-center justify-between px-8 py-4">
          <div className="flex items-center gap-4">
            <span className="text-lg font-bold text-[#f8c017]">
              Bulk Update
            </span>
          ...
          {bulkStatusError && (
            <span className="text-red-500 ml-4">{bulkStatusError}</span>
          )}
        </div>
      )}
      {/* Header */}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
            <div className="bg-[#18181b] rounded-2xl shadow-2xl w-full max-w-lg p-8 relative border border-[#f8c017]/30">
              ...
              <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2">
                <Download className="h-6 w-6 text-[#f8c017]" /> Export Orders
                Report
              </h2>
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2 mt-2">
                  <Button
                    size="sm"
                    variant={
                      !exportDateFrom && !exportDateTo ? "default" : "outline"
                    }
                    className={
                      !exportDateFrom && !exportDateTo
                        ? "bg-[#f8c017] text-black font-bold"
      ...
      {viewMode === "grid" ? (
          ...
          <table className="min-w-full bg-[#1a1a1a] border border-[#f8c017]/20 rounded-lg">
            ...
            <tbody>
              {orders.map((order) => {
                ...
                const groupDetails = order.priceTierGroupId
                  ? priceTierGroups[order.priceTierGroupId]
                  : null;
                let tierSum = 0;
                if (groupDetails && Array.isArray(groupDetails.pricingTiers)) {
                  tierSum = groupDetails.pricingTiers.reduce(
                    (sum: number, tier: any) => {
                      return (
                        sum +
                        (typeof tier.negotiatedRate === "number"
                          ? tier.negotiatedRate
                ...
                return (
                  <tr
                    key={order.id}
                    className={`border-b border-[#f8c017]/10 hover:bg-[#23232b] ${
                      isAssigned ? "bg-[#f8c017]/30" : ""
                    }`}
                  >
                    <td className="p-3">
                      <input
                    ...
                    <td className="p-3 flex gap-2">
                      ...
                      <button
                        className="text-gray-400 hover:text-[#f8c017] relative"
                      ...
                      >
                        ...
                        {order.notes && order.notes.length > 0 && (
                          <span className="absolute -top-1 -right-1 bg-[#f8c017] text-black text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center">
                            {order.notes.length}
                          </span>
                        )}
                      </button>
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            const statusInfo = getStatusInfo(order.status);
            const isCompleted = ["DELIVERED", "RETURNED", "CANCELED"].includes(
              order.status
            );
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        >
                          {formatCurrency(
                            getPriceTierSum(order.priceTierBreakdown) +
                              (order.amount || 0) +
                              (order.cost || 0),
                            currency
                          )}
                      ...
                      <div className="grid gap-2 md:grid-cols-2">
                        ...
                        {(order.priceTierBreakdown ||
                          ...
                          <div className="flex flex-col gap-1 text-gray-400">
                            {order.priceTierBreakdown && (
                              <span className="text-xs text-[#f8c017]">
                                Price Tier Breakdown:{" "}
                                {JSON.stringify(order.priceTierBreakdown)}
                              </span>
                            )}
                        ...
                        {order.note && (
                          <div className="flex items-center gap-2 text-gray-400">
                            ...
                            <span className="text-sm" title={order.note}>
                              {order.note.length > 60
                                ? order.note.slice(0, 60) + "..."
                                : order.note}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="text-sm text-gray-500">
                        ...
                        {order.assignedLogistics && (
                          <p>
                            Assigned to: {order.assignedLogistics.firstName}{" "}
                            {order.assignedLogistics.lastName}
                          </p>
                        )}
                        {order.fulfillmentWarehouse && (
                          <p>
                            Warehouse: {order.fulfillmentWarehouse.name} (
                            {order.fulfillmentWarehouse.region})
                          </p>
                        )}
                      </div>
                    </div>
                    <div className="flex flex-col gap-2 min-w-40 items-end">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="border-gray-600 text-gray-300 hover:border-[#f8c017] hover:text-[#f8c017] relative"
                        onClick={() => {
                          setSelectedOrderForNotes(order);
                          setShowNotesModal(true);
                        }}
                      ...
                      >
                        ...
                        Notes
                        {order.notes && order.notes.length > 0 && (
                          <span className="ml-1 bg-[#f8c017] text-black text-xs font-bold rounded-full px-1.5 py-0.5">
                            {order.notes.length}
                          </span>
                        )}
                      </Button>
                      ...
                      <div className="relative">
                        ...
                        {showOrderActionsMenuId === order.id && (
                          <div className="absolute right-0 mt-2 w-40 bg-[#23232b] border border-[#f8c017]/30 rounded-lg shadow-lg z-10">
                            <button
                              className="block w-full text-left px-4 py-2 text-sm text-white hover:bg-[#f8c017]/10"
                              onClick={() => handleEditOrder(order)}
                            >
                              Edit Order
                            </button>
      ...
      {selectedOrderForNotes && (
        <NotesModal
          ...
          notes={selectedOrderForNotes.notes || []}
          currentUserId={user?.id || ""}
          userRole={user?.role || ""}
          onNoteAdded={() => fetchOrders()}
          onNoteUpdated={() => fetchOrders()}
          onNoteDeleted={() => fetchOrders()}
        />
      )}
      ...
      {showOrderModal && selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-[#18181b] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-8 relative border border-[#f8c017]/30">
            <button
              className="absolute top-4 right-4 text-gray-400 hover:text-white text-xl"
                  ...
                  <span className="text-sm">
                    {selectedOrder.amount
                      ? `Price: ${formatCurrency(
                          selectedOrder.amount,
                          selectedOrder.Business?.baseCurrency || "NGN"
                        )}`
                      : ""}
                    {selectedOrder.cost
                      ? ` / Cost: ${formatCurrency(
            ...
            <div className="mb-4">
              ...
              <div className="text-white text-sm mb-2 flex items-center gap-2">
                ...
                {selectedOrder.customerPhone && (
                  <CallContactAction
                    contactNumber={selectedOrder.customerPhone}
                    contactName={selectedOrder.customerName}
                    size="icon-sm"
                  />
                )}
              </div>
              ...
              {selectedOrder.items && selectedOrder.items.length > 0 ? (
                <ul className="divide-y divide-[#f8c017]/10">
                  {selectedOrder.items.map((item, idx) => (
                    ...
                    >
                      <span className="font-mono text-yellow-400 bg-yellow-900/20 rounded px-2 py-0.5 text-xs">
                        {item.product?.sku || "-"}
                      </span>
                      <span className="text-white font-semibold">
                        {item.product?.name || "-"}
                      </span>
                      <span className="ml-auto text-[#f8c017] font-bold">
                        x{item.quantity}
                      </span>
                    </li>
                  ))}
            ...
            <div className="flex flex-col gap-2 mt-6">
              <Button
                ...
                className="border-gray-600 text-gray-300 hover:border-[#f8c017] hover:text-[#f8c017] relative"
                onClick={() => {
                  setSelectedOrderForNotes(selectedOrder);
              ...
              {isAdmin && (
                <Button
                  ...
                  disabled={
                    ["DELIVERED", "RETURNED", "CANCELED"].includes(
                      selectedOrder.status
                    ) || !!selectedOrder.assignedLogistics
                  }
                >
                  Assign Logistics
                </Button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-[#18181b] rounded-2xl shadow-2xl w-full max-w-lg p-8 relative border border-[#f8c017]/30">
            ...
            <div className="mb-4">
              ...
              <input
                className="w-full p-2 rounded bg-[#23232b] border border-gray-600 text-white"
                value={editOrderData?.customerName || ""}
                onChange={(e) =>
                  setEditOrderData((d: any) => ({
                    ...d,
