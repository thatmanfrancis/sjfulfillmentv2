...
export default function AdminOrdersPage() {
    ...
    const fetchPriceTierGroup = async (groupId: string) => {
      ...
      try {
        const data = await get(`/api/admin/price-tiers/group/${groupId}`);
        let group: any = null;
        if (data && typeof data === 'object') {
          if ('group' in data && data.group && typeof data.group === 'object' && 'id' in data.group) {
            group = data.group;
          } else if ('id' in data) {
            group = data;
          }
          ...
          <div className="bg-[#18181b] rounded-2xl shadow-2xl w-full max-w-lg p-8 relative border border-[#f8c017]/30">
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center justify-between mt-6">
                <span className="text-lg font-bold text-white">Total: {formatCurrency(
                  (getPriceTierSum(selectedOrder.priceTierBreakdown) + (selectedOrder.amount || 0) + (selectedOrder.cost || 0)),
