...
export default function AdminProductsPage() {
  ...
  const [bulkMoveQuantities, setBulkMoveQuantities] = useState<{
    [id: string]: number;
  }>({});
  ...
  const handleExport = async (format: string, options: any) => {
    try {
      ...
      toast.success(
        `Export initiated! Products will be exported as ${format.toUpperCase()}.`
      );
    } catch (error) {
  ...
  const handleWarehouseTransfer = async () => {
    ...
    try {
      ...
      const existingTargetIndex = updatedAllocations.findIndex(
        (a) => a.warehouseId === toWarehouseId
      );
      if (existingTargetIndex >= 0) {
        updatedAllocations[existingTargetIndex].allocatedQuantity +=
          transferQuantity;
      } else {
      ...
      const finalAllocations = updatedAllocations.filter(
        (a) => a.allocatedQuantity > 0
      );
      ...
      toast.success(
        `Successfully transferred ${transferQuantity} units between warehouses`
      );
    } catch (error: any) {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        stockAllocations: editForm.stockAllocations.filter(
          (sa) =>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {products.map((product) => {
            ...
            return (
              <Card
                key={product.id}
                className="relative bg-[#1a1a1a] border border-[#f8c017]/20 hover:shadow-lg hover:shadow-[#f8c017]/10 transition-all group"
              >
                ...
                <CardContent className="p-6 space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span className="truncate">
                        {product.inventory.warehouses.length} warehouse(s)
                      </span>
                    </div>
                  </div>
          ...
          <table className="w-full text-sm text-left border border-[#f8c017] rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-[#222] text-[#f8c017]">
                ...
                <th className="p-2 whitespace-nowrap">Merchant</th>
            ...
            <tbody>
              {products.map((product) => (
                ...
                >
                  ...
                  <td className="p-2 text-white whitespace-nowrap">
                    {product.inventory?.warehouses?.length ?? 0}
                  </td>
                  <td className="p-2 text-white whitespace-nowrap">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4 border-t border-gray-600 pt-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Manage stock allocations across warehouses
                    </p>
                  </div>
                  <Button
                    type="button"
                ...
                {editForm.stockAllocations.length === 0 && (
                  <div className="text-center py-4 text-gray-400 text-sm">
                    No inventory allocations. Click "Add Warehouse" to assign
                    stock to warehouses.
                  </div>
                )}
                ...
                {editForm.stockAllocations.map((allocation, index) => (
                  <div
                    key={index}
                    className="grid grid-cols-1 md:grid-cols-4 gap-3 p-3 bg-[#2a2a2a] rounded-lg"
                  >
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateStockAllocation(index, "warehouseId", value)
                        }
                      >
                ...
                {editForm.stockAllocations.length > 0 && (
                  <div className="bg-[#1a1a1a] rounded-lg p-3 border border-gray-600">
                    <div className="text-sm text-gray-300">
                      ...
                      {editForm.stockAllocations.reduce(
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <Button
                  ...
                  disabled={
                    isUpdating || !editForm.name || editForm.weightKg <= 0
                  }
                  className="bg-[#f8c017] text-black hover:bg-[#f8c017]/90"
                >
                  {isUpdating ? (
            ...
            <div className="space-y-4">
              <div className="bg-[#2a2a2a] rounded-lg p-4 border border-gray-600">
                <div className="space-y-2">
                  ...
                  <p className="text-gray-400 text-sm">
                    Business: {selectedProduct.business.name}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-[#2a2a2a] rounded-lg p-3 border border-gray-600">
                <p className="text-sm text-gray-300">
                  ...
                  {
                    warehouses.find(
                      (w) => w.id === transferAllocation.fromWarehouseId
                    )?.name
                  }
                </p>
                <p className="text-sm text-gray-300">
                  <span className="text-white font-medium">Available:</span>{" "}
                  {transferAllocation.quantity} units
                </p>
              </div>
                    ...
                    <SelectContent className="bg-white border border-gray-300">
                      {warehouses
                        .filter(
                          (w) =>
                            w.status === "ACTIVE" &&
                            w.id !== transferAllocation.fromWarehouseId
                        )
                        .map((warehouse) => (
                          <SelectItem
                            key={warehouse.id}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setTransferForm((prev) => ({
                        ...prev,
                        transferQuantity: parseInt(e.target.value) || 0,
                      }))
                    }
                    className="bg-[#2a2a2a] border-gray-600 text-white"
                    placeholder="0"
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <Button
                  ...
                  disabled={
                    ...
                    transferForm.transferQuantity <= 0 ||
                    transferForm.transferQuantity > transferAllocation.quantity
                  }
                  className="bg-[#f8c017] text-black hover:bg-[#f8c017]/90"
                >
                  Transfer Inventory
          ...
          {bulkMoveStep === 1 && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Select warehouse and quantity for each product.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                    ...
                    onClick={() => {
                      if (
                        ...
                        selectedProductIds.some(
                          (pid) =>
                            !bulkMoveQuantities[pid] ||
                            bulkMoveQuantities[pid] < 1
                        )
                      ) {
                        setBulkMoveError(
                          "Please select warehouse and enter valid quantities for all products."
                        );
                        return;
                      }
                      setBulkMoveError("");
          ...
          {bulkMoveStep === 2 && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Are you sure you want to move these products?
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="bg-[#181818] p-4 rounded border border-gray-700">
                  <div className="text-gray-300 mb-2">
                    ...
                    <span className="font-bold">
                      {warehouses.find((w) => w.id === bulkMoveWarehouse)?.name}
                    </span>
                  </div>
                  {selectedProductIds.map((pid) => {
                    const product = products.find((p) => p.id === pid);
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const moves = selectedProductIds.map((pid) => {
                          ...
                          const fromWarehouse =
                            product?.inventory.warehouses.find(
                              (w) => w.quantity > 0
                            );
                          return {
                            productId: pid,
                        ...
                        setBulkMoveError(
                          "Failed to move products. Please try again."
                        );
                      } finally {
