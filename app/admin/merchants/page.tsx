...
export default function AdminMerchantsPage() {
  ...
  const handleVerifyMerchant = async (merchantId: string) => {
    try {
      const response = await fetch(`/api/admin/merchants/${merchantId}/verify`, {
        method: 'PATCH',
  ...
  const handleSoftDeleteMerchant = async (merchantId: string) => {
    ...
    toast.warn(
      ({ closeToast }) => (
        <div>
          <p className="mb-3">Are you sure you want to deactivate this merchant account?</p>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (response.ok) {
                    ...
                    const errorData = await response.json();
                    toast.error(errorData.error || 'Failed to deactivate merchant account', {
                      position: 'top-right',
                      autoClose: 5000,
                    });
                  }
                  ...
                  toast.error('An unexpected error occurred while deactivating the account', {
                    position: 'top-right',
                    autoClose: 5000,
                  });
                }
    ...
    <div className="space-y-6 bg-black min-h-screen p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 p-1 bg-[#2a2a2a] rounded-lg">
            <Button
              ...
              className={viewMode === 'list' ? 'bg-[#f8c017] text-black' : 'text-gray-300 hover:text-white'}
            >
            ...
            <Button
              ...
              className={viewMode === 'grid' ? 'bg-[#f8c017] text-black' : 'text-gray-300 hover:text-white'}
            >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6">
        <Card className="bg-[#1a1a1a] border border-[#f8c017]/20 hover:shadow-lg hover:shadow-[#f8c017]/10 transition-all">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{stats?.totalMerchants || 0}</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#1a1a1a] border border-[#f8c017]/20 hover:shadow-lg hover:shadow-[#f8c017]/10 transition-all">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{stats?.activeMerchants || 0}</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#1a1a1a] border border-[#f8c017]/20 hover:shadow-lg hover:shadow-[#f8c017]/10 transition-all">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{stats?.inactiveMerchants || 0}</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#1a1a1a] border border-[#f8c017]/20 hover:shadow-lg hover:shadow-[#f8c017]/10 transition-all">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{stats?.newThisMonth || 0}</div>
          </CardContent>
        </Card>
      ...
      {viewMode === 'list' ? (
        <div className="space-y-4">
          {merchants.map((merchant) => (
            <Card key={merchant.id} className="bg-[#1a1a1a] border border-[#f8c017]/20 hover:shadow-lg hover:shadow-[#f8c017]/10 transition-all">
              <CardContent className="p-6">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      ...
                      <div className="flex items-center gap-2 text-gray-400">
                        ...
                        <span className="text-sm">Users: {merchant._count.users}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-400">
                        <Package className="h-4 w-4" />
                        <span className="text-sm">Products: {merchant._count.products}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-400">
                        <Calendar className="h-4 w-4" />
                        <span className="text-sm">Created: {formatDate(merchant.createdAt)}</span>
                      </div>
                    </div>

                    {/* Additional Info */}
                    {(merchant.description || merchant.address || merchant.category) && (
                      <div className="text-sm text-gray-500">
                        {merchant.description && <p>{merchant.description}</p>}
                        {merchant.address && <p>Address: {merchant.address}</p>}
                        {merchant.category && <p>Category: {merchant.category}</p>}
                      </div>
                    )}
                  </div>
                    ...
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 top-8 hidden group-hover:block w-48 bg-[#2a2a2a] border border-gray-600 rounded-lg shadow-lg z-10">
                        <div className="py-1">
                          ...
                          <button
                            onClick={() => handleSoftDeleteMerchant(merchant.id)}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="text-center p-2 bg-[#2a2a2a] rounded">
                      <div className="text-[#f8c017] font-semibold">{merchant._count.users}</div>
                      <div className="text-gray-400">Users</div>
                    </div>
                    <div className="text-center p-2 bg-[#2a2a2a] rounded">
                      <div className="text-[#f8c017] font-semibold">{merchant._count.products}</div>
      ...
      {selectedMerchant && (
        <div className={`fixed inset-0 bg-black/80 backdrop-blur-sm z-50 transition-opacity ${showMerchantDetail ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className="space-y-6">
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-white mt-1">{selectedMerchant.name}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-white mt-1">{selectedMerchant.phone || 'Not provided'}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-300">Category</label>
                        <p className="text-white mt-1">{selectedMerchant.category || 'General'}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-white mt-1">{formatDate(selectedMerchant.createdAt)}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-300">Total Users</label>
                        <p className="text-white mt-1">{selectedMerchant._count.users}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-300">Total Products</label>
                        <p className="text-white mt-1">{selectedMerchant._count.products}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedMerchant.description && (
                    <div>
                      ...
                      <p className="text-white mt-1">{selectedMerchant.description}</p>
                    </div>
                  )}
                  ...
                  <div className="border-t border-gray-700 pt-6">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      <Button
                        onClick={() => handleVerifyMerchant(selectedMerchant.id)}
                      ...
                      <Button
                        ...
                        className="border-gray-600 text-gray-300 hover:border-[#f8c017] hover:text-[#f8c017]"
                      >
                        {selectedMerchant.isActive ? '⏸ Deactivate' : '▶ Activate'} Account
                      </Button>
                      <Button
                        onClick={() => handleSoftDeleteMerchant(selectedMerchant.id)}
                        variant="outline"
