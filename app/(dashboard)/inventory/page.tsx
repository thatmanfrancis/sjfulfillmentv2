...
import { Package, TrendingDown, TrendingUp, AlertTriangle, MoreHorizontal, Plus, Search, Filter, Edit, Trash2 } from 'lucide-react';
...
async function getInventoryData(): Promise<InventoryData> {
  try {
    // For now, we'll use the products API since inventory-specific API might not be fully implemented
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/products?includeInventory=true`, {
      cache: 'no-store',
      headers: {
    ...
    // Transform product data to inventory format
    const inventory = data.products?.map((product: any) => ({
      id: `${product.id}-inventory`,
      productId: product.id,
      warehouseId: 'default-warehouse',
      quantity: Math.floor(Math.random() * 1000) + 50, // Mock data
    ...
    <div className="min-h-screen bg-brand-black p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-brand-gold">Inventory Management</h1>
            <p className="text-gray-400">Track and manage stock levels across all warehouses</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
          <Card className="bg-gray-900/50 border-gray-800 shadow-lg">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Package className="h-4 w-4 text-gradient-gold" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-gradient-gold">{data.summary.totalItems}</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gray-900/50 border-gray-800 shadow-lg">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-300">Reserved</CardTitle>
        ...
        <Card className="bg-gray-900/50 border-gray-800 shadow-lg">
          <CardHeader>
            <CardTitle className="text-gradient-gold">Filter Inventory</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-gray-900/50 border-gray-800 shadow-lg">
          <CardHeader>
            <CardTitle className="text-gradient-gold">Current Inventory</CardTitle>
            ...
            <CardContent className="py-12">
              <div className="text-center">
                ...
                <p className="mt-2 text-sm text-gray-400">
                  Start by adding products to your warehouses.
                </p>
