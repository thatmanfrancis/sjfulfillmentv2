...
export async function GET(request: NextRequest) {
  try {
    ...
    const businessesChange = calculateChange(activeBusinesses, previousBusinesses);

    // Process order status distribution
    const orderStatusMap = new Map(ordersByStatus.map(item => [item.status, item._count.id]));
    const getOrderCount = (status: string) => orderStatusMap.get(status as any) || 0;

    const transformedOrders = recentOrders.map((order: any) => ({
      id: order.id,
      business: order.Business?.name || 'Unknown Business',
      customer: `${order.User?.firstName || 'Unknown'} ${order.User?.lastName || 'User'}`,
    ...
    const avgOrderValue = totalOrders > 0 ? currentTotalRevenue / totalOrders : 0;
    const previousAvgOrderValue = previousOrders > 0 ? previousTotalRevenue / previousOrders : 0;
