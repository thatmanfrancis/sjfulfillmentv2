...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify admin or logistics permissions
    ...
    if (!user || !["ADMIN", "LOGISTICS"].includes(user.role)) {
      return NextResponse.json(
        { error: "Admin or Logistics permissions required" },
        { status: 403 }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalStock = warehouses.reduce(
      (sum: number, w: any) =>
        sum +
        w.StockAllocation.reduce(
          (stockSum: number, sa: any) => stockSum + sa.allocatedQuantity,
          0
        ),
      0
    );
    ...
    const stats = {
      ...
      inactiveWarehouses: warehouses.filter((w) => w.status === "INACTIVE")
        .length,
      maintenanceWarehouses: warehouses.filter(
    ...
    const warehousesData = warehouses.map((warehouse) => ({
      ...
      name: warehouse.name,
      location:
        `${warehouse.city || ""}, ${warehouse.state || ""}`.replace(
          /^,\s*|,\s*$/g,
          ""
        ) || warehouse.region,
      ...
      currentStock: warehouse.StockAllocation.reduce(
        (sum: number, sa: any) => sum + sa.allocatedQuantity,
        0
      ),
